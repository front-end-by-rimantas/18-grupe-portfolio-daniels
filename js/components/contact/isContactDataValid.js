function isContactDataValid(data) {
    let errors = [];
    ...
    if (typeof data.contact.title !== 'string') {
        console.log('menu title must be string.');
        return false;
    }
    if (data.contact.title.length < 2 || data.contact.title.length > 15) {
        errors.push('menu title length must be 2-15 symbols.');
    }
    if (!/^[a-zA-Z]+$/.test(data.contact.title)) {
